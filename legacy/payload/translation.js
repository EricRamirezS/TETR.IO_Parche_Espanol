...
const translations = {
    ...
    "footer_text": (e) => {
        ...
        e.childNodes.forEach(node => {
            ...
            const nodeTranslated = nodeValue
                .replace(
                    "EXPERT QUICK PLAY played by",
                    "PARTIDA RAPIDA EXPERTO jugada por"
                )
                .replace(
                    "QUICK PLAY played by",
                    "PARTIDA RAPIDA jugada por"
                )
                .replace(
                    "40 LINES played by",
                    "40 LINEAS jugadas por"
                )
                .replace(
                    "played on",
                    "jugaron el"
    ...
    "viewreplay_endleague": (e) => text(e, "REPETICION"),
    ...
    "league_standing_container": (e) => {
        ...
        if (h1) text(h1, "CLASIFICACION DE TETRA LEAGUE");
    },
    ...
    "ongoing": (e) => {
        ...
        e.querySelectorAll(".ongoing_sub, .ongoing_button").forEach(el => {
            const value = el.textContent.trim();
            const translated = translations[value];
            ...
            notification.querySelectorAll("p").forEach(p => {
                ...
                p.querySelectorAll("b").forEach(b => {
                    ...
                    if (value === "friended you back") {
                        const translated = " aceptó la solicitud de amistad.";
    ...
    "dialogs": (e) => {
        const translate = (selector, translations) => {
            e.querySelectorAll(selector).forEach(el => {
                ...
                let translated =
                    translations[value] ||
                    translations[value.toUpperCase()];
        ...
        e.querySelectorAll(".supporter_modal").forEach(modal => {
            ...
            if (bumper) {
                const textNodes = Array.from(bumper.childNodes).filter(
                    node => node.nodeType === Node.TEXT_NODE
                );
                ...
                textNodes.forEach(node => {
                    ...
                    value = value.replace(
                        "TETR.IO will always be ",
                        "TETR.IO siempre sera "
                    );
            ...
            if (payment) {
                payment.querySelectorAll("p").forEach(p => {
                    ...
                    if (value.startsWith("TETR.IO Supporter is a ")) {
                        const translated =
                            "TETR.IO Supporter es un pago unico. Si lo compras mientras sigues siendo Supporter, extendera la duracion de tu estado actual de Supporter.";
                ...
                if (custom) {
                    ...
                    const translated =
                        value.replace(
                            ...
                            "...o elige una duracion personalizada"
                        );
        ...
        translate(".oob_modal h1", {
            ...
            "KICKED BY ROOM OWNER": "EXPULSADO POR EL DUEÑO DE LA SALA",
        ...
        e.querySelectorAll(".tetra_topbutton").forEach(button => {
            ...
            const node = [...button.childNodes].find(
                node => node.nodeType === Node.TEXT_NODE
            );
        ...
        e.querySelectorAll(".tetra_modal_record").forEach(record => {
            const header = record.querySelector(
        ...
        attrTranslate(
            ".tetra_modal_record h5, .tetra_modal_record h3",
            {
        ...
        const anonymousWarning = e.querySelector(
            ".tetra_modal_warning"
        );
        ...
        e.querySelectorAll(".crash_modal").forEach(dialog => {
            ...
            if (paragraphs[0]) {
                paragraphs[0].childNodes.forEach(node => {
                    ...
                    const translated = value
                        .replace(
                            "no seriously, are you sure you want to delete",
                            "no, en serio, ¿estas seguro de que quieres eliminar"
                        )
                        .replace(
                            "and EVERYTHING INVOLVED, like your replays, badges, XP, rating, etc?",
                            "y TODO LO RELACIONADO, como tus repeticiones, insignias, XP, puntuacion, etc.?"
                        );
            ...
            if (
                modalAlso &&
                modalAlso.textContent.trim() === "you can never get it back!!!"
            ) {
